/**
 * gosetups.gg scraper CLI wrapper.
 *
 * Round 10. The actual scrape logic lives in lib/scrape/gosetups.ts so it can
 * be called from the production /api/ingest route. This wrapper preserves the
 * developer flow `npm run scrape:gosetups` for local runs against ./dev.db
 * (or DATABASE_PATH).
 */
